...
import { toeflGlossary } from "@/data/toeflGlossaryData";
import { speakText } from "@/utils/speechUtils";

export const ToeflGlossary = () => {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedCategory, setSelectedCategory] = useState<string>("all");

  const categories = ["all", ...Array.from(new Set(toeflGlossary.map((term) => term.category)))];

  const filteredTerms = toeflGlossary.filter((term) => {
    const matchesSearch =
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl">
            📖 Glossário TOEFL / TOEFL Glossary
          </CardTitle>
          <p className="text-muted-foreground">
            Termos essenciais para o exame TOEFL / Essential TOEFL exam terms
          </p>
        </CardHeader>
      </Card>
